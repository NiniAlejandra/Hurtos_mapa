...
def generate_html():
    ...
    for record in data_json:
        if 'FECHA HURTO' in record and pd.notna(record['FECHA HURTO']):
            record['FECHA_HURTO'] = record['FECHA HURTO'].strftime('%d/%m/%Y')
        else:
            record['FECHA_HURTO'] = ''
...
<body>
    ...
    <script>
        // Datos de hurtos
        const hurtosData = {json.dumps(data_json, ensure_ascii=False, default=str)};
        ...
        function updateMap(selectedYears, mapType) {{
            ...
            if (mapType === 'heatmap' || mapType === 'both') {{
                traces.push({{
                    type: 'densitymap',
                    lat: filteredData.map(d => parseFloat(d.LATITUD)),
                    lon: filteredData.map(d => parseFloat(d.LONGITUD)),
            ...
            if (mapType === 'points' || mapType === 'both') {{
                ...
                selectedYears.forEach(año => {{
                    ...
                    if (añoData.length > 0) {{
                        traces.push({{
                            type: 'scattermap',
                            lat: añoData.map(d => parseFloat(d.LATITUD)),
                            lon: añoData.map(d => parseFloat(d.LONGITUD)),
                            mode: 'markers',
                            marker: {{
                                size: 8,
                                color: colorMap[año],
                            ...
                            text: añoData.map(d => `${{d.STICKER || 'N/A'}}<br>${{d.DIRECCION || 'N/A'}}<br>${{d.FECHA_HURTO || 'N/A'}}`),
                            hoverinfo: 'text',
                            name: `Año ${{año}}`
                        }});
                    }}
